...
import LifecyclePanel from "@/components/common/lifecycle/LifecyclePanel";//IMPORT

const EDIT_URL = "/generated/region/edit/";

const NAV_ITEMS = [
    {id: "informations", label: "Informations", Icon: InfoOutlined},
    {id: "statut", label: "Cycle de vie", Icon: TimelineOutlined},// ── Navs ─
];


// ── Main component ────────────────────────────────────────

export default function CommandeInfo({uid}) {
    ...
    const loadEntity = () => {
        if (!uid) return;
        setLoading(true);
        CommandeService.detailsEntity(uid).then((response) => {
    ...
    const makePatcher = (fieldKey) => async (value) => {
        const res = await CommandeService.patchEntity(uid, {[fieldKey]: value});
            ...
            <div className={p.root}>
                ...
                <div className={p.page}>
                    ...
                    >
                     ...
                     <LifecyclePanel
    lifecycleManaged={el}
    postUpdateStateSuccess={loadEntity}
/>
{/* Behaviours */}
