...
const BUSINESS_TYPE = "com.katappult.online.types.LignePanierType";
const LIST_URL      = "/generated/lignepanier";


// ── Main component ────────────────────────────────────────

export default function LignePanierForm({ uid }) {
    ...
    useEffect(() => {
        if (!uid) return;
        setIsFetching(true);
        LignePanierService.detailsEntity(uid)
            .then((response) => {
...
// FORM DATAS
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setIsLoading(true);
            const response = uid
                ? await LignePanierService.updateEntity(uid, formData)
                : await LignePanierService.createEntity(formData);
