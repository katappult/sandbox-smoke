...
export default function CategorieInfo({uid}) {
    ...
    const loadEntity = () => {
        if (!uid) return;
        setLoading(true);
        CategorieService.detailsEntity(uid).then((response) => {
    ...
    const makePatcher = (fieldKey) => async (value) => {
        const res = await CategorieService.patchEntity(uid, {[fieldKey]: value});
