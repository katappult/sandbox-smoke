...
const BUSINESS_TYPE = "com.katappult.online.types.CategorieType";
const LIST_URL      = "/generated/categorie";


// ── Main component ────────────────────────────────────────

export default function CategorieForm({ uid }) {
    ...
    useEffect(() => {
        if (!uid) return;
        setIsFetching(true);
        CategorieService.detailsEntity(uid)
            .then((response) => {
                if (responseSuccess(response)) {
                    const details = response.data;
                    setFullId(details.fullId);
                    form.setFieldValue("uid", details.uid);
                    			form.setFieldValue("titre", details.titre);
form.setFieldValue("allIllustrations", details.allIllustrations);// FORM DATAS
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setIsLoading(true);
            const response = uid
                ? await CategorieService.updateEntity(uid, formData)
                : await CategorieService.createEntity(formData);
                ...
                <FormSection icon={<UserOutlined />} title="Informations">
                    <div className={FormStyle.grid_1}>
                        {/*FormRoot*/}
                                    ...
                                    {renderFormItem("titre", "Titre", true, "text", "Titre")}
{/*Form*/}
                    </div>
                </FormSection>
