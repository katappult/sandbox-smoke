...
import CategorieDetailsDrawer from "@/components/generated/categorie/CategorieDrawer";
...
{/*table columns*/},
    ...
    const doRequestLoadItems = async (page) => {
        ...
        const response = await CategorieService.listEntity(page - 1, PAGE_SIZE, sort, "", statusFilter);
    ...
    const searchItems = async (searchTerm) => {
        ...
        const sort = getSortDefinitionOfTable(sortingFromDatatable);
        const statusFilter = selectedStatuses.length === 0 ? ALL : selectedStatuses.join(",");
        const response = await CategorieService.searchEntity(0, PAGE_SIZE, searchTerm, sort, statusFilter);
    ...
    const deleteItem = async (uid, event) => {
        ...
        const response = await CategorieService.deleteEntity(uid);
        if (!responseSuccess(response)) {
            notificationErrorDeleting();
        } else {
    ...
    return (
        <>
            ...
            {isDrawerVisible && <CategorieDetailsDrawer
