...
import PanierDetailsDrawer from "@/components/generated/panier/PanierDrawer";
...
const INFO_PAGE_URL = "/generated/panier/info/";
const EDIT_PAGE_URL = "/generated/panier/edit/";
const ADD_PAGE_URL = "/generated/panier/add";
...
export default function PanierList() {
    ...
    const doRequestLoadItems = async (page) => {
        ...
        const response = await PanierService.listEntity(page - 1, PAGE_SIZE, sort, "", statusFilter);
    ...
    const searchItems = async (searchTerm) => {
        ...
        const sort = getSortDefinitionOfTable(sortingFromDatatable);
        const statusFilter = selectedStatuses.length === 0 ? ALL : selectedStatuses.join(",");
        const response = await PanierService.searchEntity(0, PAGE_SIZE, searchTerm, sort, statusFilter);
    ...
    const deleteItem = async (uid, event) => {
        ...
        const response = await PanierService.deleteEntity(uid);
        if (!responseSuccess(response)) {
            notificationErrorDeleting();
        } else {
    ...
    return (
        <>
            ...
            {isDrawerVisible && <PanierDetailsDrawer
