...
const BUSINESS_TYPE = "com.katappult.online.types.LigneCommandeType";
const LIST_URL      = "/generated/lignecommande";


// ── Main component ────────────────────────────────────────

export default function LigneCommandeForm({ uid }) {
    ...
    useEffect(() => {
        if (!uid) return;
        setIsFetching(true);
        LigneCommandeService.detailsEntity(uid)
            .then((response) => {
                if (responseSuccess(response)) {
                    const details = response.data;
                    setFullId(details.fullId);
                    form.setFieldValue("uid", details.uid);
                    			form.setFieldValue("quantite", details.quantite);
...
// FORM DATAS
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            setIsLoading(true);
            const response = uid
                ? await LigneCommandeService.updateEntity(uid, formData)
                : await LigneCommandeService.createEntity(formData);
                ...
                <FormSection icon={<UserOutlined />} title="Informations">
                    <div className={FormStyle.grid_1}>
                        {/*FormRoot*/}
                                    ...
                                    {renderFormItem("quantite", "Quantite", true, "number", "Quantite")}
