...
import {LigneCommandeService} from "@/services/generated/LigneCommande.services";
...
import LigneCommandeDetailsDrawer from "@/components/generated/lignecommande/LigneCommandeDrawer";
...
const INFO_PAGE_URL = "/generated/lignecommande/info/";
const EDIT_PAGE_URL = "/generated/lignecommande/edit/";
const ADD_PAGE_URL = "/generated/lignecommande/add";
...
export default function LigneCommandeList() {
    ...
    const column = [
        ...
        {/*table columns_before*/},
          ...
          dataIndex: "quantite",
          title: "Quantite",
          sorter:{column: "quantite"}
     },
     {
          key: "prixUnitaire",
          dataIndex: "prixUnitaire",
          title: "Prix Unitaire",
          sorter:{column: "prixUnitaire"}
     },
{/*table columns*/},
        {
            key: "action",
    ...
    const doRequestLoadItems = async (page) => {
        ...
        const response = await LigneCommandeService.listEntity(page - 1, PAGE_SIZE, sort, "", statusFilter);
    ...
    const searchItems = async (searchTerm) => {
        ...
        const sort = getSortDefinitionOfTable(sortingFromDatatable);
        const statusFilter = selectedStatuses.length === 0 ? ALL : selectedStatuses.join(",");
        const response = await LigneCommandeService.searchEntity(0, PAGE_SIZE, searchTerm, sort, statusFilter);
    ...
    const deleteItem = async (uid, event) => {
        ...
        const response = await LigneCommandeService.deleteEntity(uid);
        if (!responseSuccess(response)) {
            notificationErrorDeleting();
        } else {
    ...
    return (
        <>
            ...
            {isDrawerVisible && <LigneCommandeDetailsDrawer
