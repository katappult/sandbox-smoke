...
export default function LigneCommandeInfo({uid}) {
    ...
    const loadEntity = () => {
        if (!uid) return;
        setLoading(true);
        LigneCommandeService.detailsEntity(uid).then((response) => {
    ...
    const makePatcher = (fieldKey) => async (value) => {
        const res = await LigneCommandeService.patchEntity(uid, {[fieldKey]: value});
