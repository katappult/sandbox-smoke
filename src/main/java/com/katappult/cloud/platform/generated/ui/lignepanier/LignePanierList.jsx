...
import {LignePanierService} from "@/services/generated/LignePanier.services";
...
import LignePanierDetailsDrawer from "@/components/generated/lignepanier/LignePanierDrawer";
...
const INFO_PAGE_URL = "/generated/lignepanier/info/";
const EDIT_PAGE_URL = "/generated/lignepanier/edit/";
const ADD_PAGE_URL = "/generated/lignepanier/add";
...
export default function LignePanierList() {
...
{/*table columns*/},
    ...
    const doRequestLoadItems = async (page) => {
        ...
        const response = await LignePanierService.listEntity(page - 1, PAGE_SIZE, sort, "", statusFilter);
    ...
    const searchItems = async (searchTerm) => {
        ...
        const sort = getSortDefinitionOfTable(sortingFromDatatable);
        const statusFilter = selectedStatuses.length === 0 ? ALL : selectedStatuses.join(",");
        const response = await LignePanierService.searchEntity(0, PAGE_SIZE, searchTerm, sort, statusFilter);
    ...
    const deleteItem = async (uid, event) => {
        ...
        const response = await LignePanierService.deleteEntity(uid);
        if (!responseSuccess(response)) {
            notificationErrorDeleting();
        } else {
    ...
    return (
        <>
            ...
            {isDrawerVisible && <LignePanierDetailsDrawer
