...
import {CommandeService} from "_services/_generated/Commande.services";
import {DataTable, WaitingPane, EmptyPane} from "_components/_common";
import {coreUri} from '_helpers/CoreUri';
import { commons } from '_helpers/commons.js';

const ROOT_ADMIN_URL = '/admin?tab=home&view=commande';

function CommandeInverseOneToMany(props)  {
    ...
    const loadItem = (pageIndex) => {
        setLoading(true)
        CommandeService.getOneToManyLigneCommandeInverse(props.entityId).then(response => {
    ...
    const doForwardDetails = (e, id) => {
        ...
        const url = coreUri.backOfficeViewURL(tabName, 'commande', ["rootId=" +  id]);
...
export default CommandeInverseOneToMany;
