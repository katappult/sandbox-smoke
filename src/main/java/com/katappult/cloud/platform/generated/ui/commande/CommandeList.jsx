...
import CommandeDetailsDrawer from "@/components/generated/commande/CommandeDrawer";
...
const INFO_PAGE_URL = "/generated/commande/info/";
const EDIT_PAGE_URL = "/generated/commande/edit/";
const ADD_PAGE_URL = "/generated/commande/add";
...
{/*table columns_before*/},
          ...
          title: "Date Commande",
          sorter:{column: "dateCommande"}
     },
     {
          key: "montantTotal",
          dataIndex: "montantTotal",
          title: "Montant Total",
          sorter:{column: "montantTotal"}
     },
{
    key: "status",
    dataIndex: "status",
...
{/*table columns*/},
    ...
    const doRequestLoadItems = async (page) => {
        ...
        const response = await CommandeService.listEntity(page - 1, PAGE_SIZE, sort, "", statusFilter);
    ...
    const searchItems = async (searchTerm) => {
        ...
        const sort = getSortDefinitionOfTable(sortingFromDatatable);
        const statusFilter = selectedStatuses.length === 0 ? ALL : selectedStatuses.join(",");
        const response = await CommandeService.searchEntity(0, PAGE_SIZE, searchTerm, sort, statusFilter);
    ...
    const deleteItem = async (uid, event) => {
        ...
        const response = await CommandeService.deleteEntity(uid);
        if (!responseSuccess(response)) {
            notificationErrorDeleting();
        } else {
    ...
    return (
        <>
            ...
            {isDrawerVisible && <CommandeDetailsDrawer
