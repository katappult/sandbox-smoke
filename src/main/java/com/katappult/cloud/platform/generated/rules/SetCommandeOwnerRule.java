...
@AllArgsConstructor
@Service
public class SetCommandeOwnerRule implements IVetoableBusinessRule {
    ...
    public void apply(KatappultEvent event) {
        ...
        if(event.getUiAttributes() != null && event.getUiAttributes().getAllAttributesFromUI() != null) {
            String ownerId = (String) event.getUiAttributes().getAllAttributesFromUI().get("ownerId");
            if(StringUtils.isNotBlank(ownerId)) {

                Commande entity = (Commande) event.getSubject();

                String ownerIdDecoded = ObjectIdentifierUtils.decode(ownerId);
                UserAccount roleB = (UserAccount) persistableService.findById(ownerIdDecoded);
                entity.setOwner(roleB);
