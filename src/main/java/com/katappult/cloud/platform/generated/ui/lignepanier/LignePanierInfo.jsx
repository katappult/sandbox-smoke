...
export default function LignePanierInfo({uid}) {
    ...
    const loadEntity = () => {
        if (!uid) return;
        setLoading(true);
        LignePanierService.detailsEntity(uid).then((response) => {
    ...
    const makePatcher = (fieldKey) => async (value) => {
        const res = await LignePanierService.patchEntity(uid, {[fieldKey]: value});
