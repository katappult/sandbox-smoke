...
import com.katappult.cloud.platform.generated.model.rest.CategorieRestRequest;
...
class CategorieIntegrationTests extends AbstractGeneratedTests{

    private static final String ROOT_URL = "categorie";

    private String createdCategorieId;
    ...
    void login() throws Exception {
        ...
        createdCategorieId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("uid").asText();
        assertNotNull(createdCategorieId);
    }
    ...
    void getDetails() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/" + createdCategorieId)
                        .header("Authorization",  adminToken))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.uid").value(createdCategorieId))
                .andDo(print());
    }
    ...
    void updateElement() throws Exception {
        CategorieRestRequest updated = buildRequest();

        mockMvc.perform(put(ROOT_URL + "/" + createdCategorieId)
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listFromUids() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/listFromUids")
                        .header("Authorization",  adminToken)
                        .param("identifier", createdCategorieId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    void advancedSearchEmpty() throws Exception {
        mockMvc.perform(post(ROOT_URL + "/advanced_search")
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new CategorieQuerySpec())))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    private CategorieRestRequest buildRequest() {
        CategorieRestRequest request = new CategorieRestRequest();
        request.setTitre(randomString());
    
        request.getParams().put("businessType", "com.katappult.online.types.CategorieType");
        return request;
    }
}
