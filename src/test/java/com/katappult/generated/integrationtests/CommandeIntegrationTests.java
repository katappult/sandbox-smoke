...
import com.katappult.cloud.platform.generated.model.rest.CommandeRestRequest;
...
class CommandeIntegrationTests extends AbstractGeneratedTests{

    private static final String ROOT_URL = "commande";

    private String createdCommandeId;
    ...
    void login() throws Exception {
        ...
        createdCommandeId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("uid").asText();
        assertNotNull(createdCommandeId);
    }
    ...
    void getDetails() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/" + createdCommandeId)
                        .header("Authorization",  adminToken))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.uid").value(createdCommandeId))
                .andDo(print());
    }
    ...
    void updateElement() throws Exception {
        CommandeRestRequest updated = buildRequest();

        mockMvc.perform(put(ROOT_URL + "/" + createdCommandeId)
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listFromUids() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/listFromUids")
                        .header("Authorization",  adminToken)
                        .param("identifier", createdCommandeId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    void advancedSearchEmpty() throws Exception {
        mockMvc.perform(post(ROOT_URL + "/advanced_search")
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new CommandeQuerySpec())))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    private CommandeRestRequest buildRequest() {
        CommandeRestRequest request = new CommandeRestRequest();
        request.setMontantTotal(randomInt());
    
        request.getParams().put("businessType", "com.katappult.online.types.CommandeType");
        return request;
    }
}
