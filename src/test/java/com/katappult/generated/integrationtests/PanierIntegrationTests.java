...
import com.katappult.cloud.platform.generated.model.rest.PanierRestRequest;
...
class PanierIntegrationTests extends AbstractGeneratedTests{

    private static final String ROOT_URL = "panier";

    private String createdPanierId;
    ...
    void login() throws Exception {
        ...
        createdPanierId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("uid").asText();
        assertNotNull(createdPanierId);
    }
    ...
    void getDetails() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/" + createdPanierId)
                        .header("Authorization",  adminToken))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.uid").value(createdPanierId))
                .andDo(print());
    }
    ...
    void updateElement() throws Exception {
        PanierRestRequest updated = buildRequest();

        mockMvc.perform(put(ROOT_URL + "/" + createdPanierId)
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listFromUids() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/listFromUids")
                        .header("Authorization",  adminToken)
                        .param("identifier", createdPanierId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    void advancedSearchEmpty() throws Exception {
        mockMvc.perform(post(ROOT_URL + "/advanced_search")
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new PanierQuerySpec())))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    private PanierRestRequest buildRequest() {
        PanierRestRequest request = new PanierRestRequest();
        
        request.getParams().put("businessType", "com.katappult.online.types.PanierType");
        return request;
    }
}
