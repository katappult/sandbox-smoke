...
import com.katappult.cloud.platform.generated.model.rest.LigneCommandeRestRequest;
...
class LigneCommandeIntegrationTests extends AbstractGeneratedTests{

    private static final String ROOT_URL = "ligne_commande";

    private String createdLigneCommandeId;
    ...
    void login() throws Exception {
        ...
        createdLigneCommandeId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("uid").asText();
        assertNotNull(createdLigneCommandeId);
    }
    ...
    void getDetails() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/" + createdLigneCommandeId)
                        .header("Authorization",  adminToken))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.uid").value(createdLigneCommandeId))
                .andDo(print());
    }
    ...
    void updateElement() throws Exception {
        LigneCommandeRestRequest updated = buildRequest();

        mockMvc.perform(put(ROOT_URL + "/" + createdLigneCommandeId)
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listFromUids() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/listFromUids")
                        .header("Authorization",  adminToken)
                        .param("identifier", createdLigneCommandeId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    void advancedSearchEmpty() throws Exception {
        mockMvc.perform(post(ROOT_URL + "/advanced_search")
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new LigneCommandeQuerySpec())))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    private LigneCommandeRestRequest buildRequest() {
        LigneCommandeRestRequest request = new LigneCommandeRestRequest();
        request.setQuantite(randomInt());
    request.setPrixUnitaire(randomInt());
    
        request.getParams().put("businessType", "com.katappult.online.types.LigneCommandeType");
        return request;
    }
}
