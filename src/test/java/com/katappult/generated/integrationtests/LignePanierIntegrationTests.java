...
import com.katappult.cloud.platform.generated.model.rest.LignePanierRestRequest;
...
class LignePanierIntegrationTests extends AbstractGeneratedTests{

    private static final String ROOT_URL = "ligne_panier";

    private String createdLignePanierId;
    ...
    void login() throws Exception {
        ...
        createdLignePanierId = objectMapper.readTree(createResult.getResponse().getContentAsString()).get("uid").asText();
        assertNotNull(createdLignePanierId);
    }
    ...
    void getDetails() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/" + createdLignePanierId)
                        .header("Authorization",  adminToken))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.uid").value(createdLignePanierId))
                .andDo(print());
    }
    ...
    void updateElement() throws Exception {
        LignePanierRestRequest updated = buildRequest();

        mockMvc.perform(put(ROOT_URL + "/" + createdLignePanierId)
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void listFromUids() throws Exception {
        mockMvc.perform(get(ROOT_URL + "/listFromUids")
                        .header("Authorization",  adminToken)
                        .param("identifier", createdLignePanierId))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    void advancedSearchEmpty() throws Exception {
        mockMvc.perform(post(ROOT_URL + "/advanced_search")
                        .header("Authorization",  adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new LignePanierQuerySpec())))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value("SUCCESS"))
                .andExpect(jsonPath("$.dataList").isArray())
    ...
    private LignePanierRestRequest buildRequest() {
        LignePanierRestRequest request = new LignePanierRestRequest();
        request.setQuantite(randomInt());
    
        request.getParams().put("businessType", "com.katappult.online.types.LignePanierType");
        return request;
    }
}
